...
import { getBackdropUrl, Movie, TVShow } from "@/lib/tmdb";
...
export function HeroCarousel({ items, type }: HeroCarouselProps) {
  ...
  const overview = currentItem.overview || "لا يوجد وصف متاح";
  const rating = currentItem.vote_average?.toFixed(1) || "N/A";
  const link = type === "movie" ? `/watch/${currentItem.id}` : `/watch-tv/${currentItem.id}`;
